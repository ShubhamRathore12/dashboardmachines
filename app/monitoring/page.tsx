...
export default function MonitoringPage() {
  ...
  return (
    <div ref={containerRef} className="p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold mb-8">Monitoring Location Overview</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {monitoringData.map((item, index) => (
          <Card key={index} className="monitoring-card p-6 hover:shadow-lg transition-shadow">
...
function StatusIndicator({ label, status }: { label: string; status: 'active' | 'inactive' | 'warning' }) {
  const colors = {
    active: 'bg-green-500',
